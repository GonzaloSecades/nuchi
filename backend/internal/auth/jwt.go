...
// signed with any other algorithm (so a token forged with "alg": "none" or
// an attacker-chosen algorithm is never accepted), expired tokens, and
// tokens whose subject is not a valid UUID. This is the helper #43 wires
// into resource-route middleware to derive the authenticated user.
